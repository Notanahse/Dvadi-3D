...
@onready var camara_principal: Camera3D = $CameraPov
@onready var camara_alt: Camera3D = $Camera3D
var camara_actual := 0
...
func _input(event):
	...
	if event.is_action_pressed("cambiarcamara2"):
...
func _physics_process(delta):
	...
	if Input.is_key_pressed(KEY_UP): direccion.z -= 1
	if Input.is_key_pressed(KEY_DOWN): direccion.z += 1
	if Input.is_key_pressed(KEY_LEFT): direccion.x -= 1
	if Input.is_key_pressed(KEY_RIGHT): direccion.x += 1
...
func patear_pelota():
	...
	var dir = -get_camara_activa().global_transform.basis.z
	dir = dir.normalized()
